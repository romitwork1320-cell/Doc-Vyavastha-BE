// Package categories implements /api/StudentCategories (tenant-scoped CRUD).
//
...
// Handler serves the StudentCategories endpoints.
type Handler struct {
	tm     *tenancy.Manager
	logger *slog.Logger
	mw     *middleware.Auth
...
func (h *Handler) Mount(r chi.Router) {
	r.Route("/StudentCategories", func(r chi.Router) {
		r.Get("/", h.list)
		r.With(h.mw.RequirePermission("/student-categories", "CanAdd")).Post("/", h.create)
		r.Get("/{id}", h.get)
		r.With(h.mw.RequirePermission("/student-categories", "CanEdit")).Put("/{id}", h.update)
		r.With(h.mw.RequirePermission("/student-categories", "CanDelete")).Delete("/{id}", h.delete)
	})
}
...
func toDTO(c tenant.StudentCategory) dto {
	return dto{
		ID:          c.ID.String(),
		Name:        c.Name,
...
func (h *Handler) list(w http.ResponseWriter, r *http.Request) {
	...
	err := h.tm.InTenantTx(ctx, reqctx.Schema(ctx), func(q *tenant.Queries) error {
		rows, err := q.ListStudentCategories(ctx, tenant.ListStudentCategoriesParams{
			Filter: page.FilterValue(), Lim: page.Limit(), Off: page.Offset(),
		})
		if err != nil {
			return err
		}
		total, err = q.CountStudentCategories(ctx, page.FilterValue())
		if err != nil {
			return err
		}
...
func (h *Handler) get(w http.ResponseWriter, r *http.Request) {
	...
	var out dto
	err = h.tm.InTenantTx(ctx, reqctx.Schema(ctx), func(q *tenant.Queries) error {
		c, err := q.GetStudentCategory(ctx, id)
		if err != nil {
			return err
		}
		out = toDTO(c)
	...
	if errors.Is(err, pgx.ErrNoRows) {
		apiresp.NotFound(w, "Category not found")
		return
	}
	if err != nil {
		h.fail(w, err)
...
func (h *Handler) create(w http.ResponseWriter, r *http.Request) {
	...
	var out dto
	err := h.tm.InTenantTx(ctx, reqctx.Schema(ctx), func(q *tenant.Queries) error {
		c, err := q.CreateStudentCategory(ctx, tenant.CreateStudentCategoryParams{
			Name:        req.Name,
	...
	apiresp.Created(w, out, "Category created")
}
...
func (h *Handler) update(w http.ResponseWriter, r *http.Request) {
	...
	var out dto
	err = h.tm.InTenantTx(ctx, reqctx.Schema(ctx), func(q *tenant.Queries) error {
		c, err := q.UpdateStudentCategory(ctx, tenant.UpdateStudentCategoryParams{
			Name:        req.Name,
			Description: conv.PtrStr(req.Description),
			Status:      statusOrDefault(req.Status),
			ID:          id,
	...
	apiresp.OK(w, out, "Category updated")
}
...
func (h *Handler) delete(w http.ResponseWriter, r *http.Request) {
	...
	err = h.tm.InTenantTx(ctx, reqctx.Schema(ctx), func(q *tenant.Queries) error {
		var derr error
		rows, derr = q.DeleteStudentCategory(ctx, id)
		return derr
	})
	if err != nil {
		var pgErr *pgconn.PgError
		if errors.As(err, &pgErr) && pgErr.Code == "23503" {
			apiresp.Conflict(w, "Cannot delete category because it is already used by students or other records.")
			return
		}
		h.fail(w, err)
		return
	...
	apiresp.OK(w, true, "Category deleted")
}

func (h *Handler) fail(w http.ResponseWriter, err error) {
	h.logger.Error("StudentCategories handler error", "err", err)
